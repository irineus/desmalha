#!/usr/bin/env bash
# tool/setup_env.sh — bootstrap idempotente do ambiente de build do Desmalha (Linux).
#
...
#     DESMALHA_SKIP_ANDROID=1 pula a parte Android (permite analyze/test sem build apk).
set -euo pipefail
...
# -------------------------------------------------------------- Android SDK
if [ "${DESMALHA_SKIP_ANDROID:-0}" = "1" ]; then
  log "Android SDK PULADO (DESMALHA_SKIP_ANDROID=1) — build apk indisponível"
else
  log "Android SDK"
